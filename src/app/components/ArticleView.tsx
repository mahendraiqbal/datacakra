...
const ArticleView: React.FC = () => {
  ...
  useEffect(() => {
    const fetchArticles = async () => {
      try {
        const response = await fetch('https://extra-brooke-yeremiadio-46b2183e.koyeb.app/api/articles?populate=*');
  ...
  const handleComment = async (articleId: number) => {
    ...
    if (comment) {
      try {
        ...
        const response = await axiosInstance.post('https://extra-brooke-yeremiadio-46b2183e.koyeb.app/api/comments', {
          data: {
            content: comment,
            article: articleId,
          },
  ...
  // Handle edit comment
  const handleEditComment = async (documentId: string, currentContent: string) => {
    const token = localStorage.getItem("token");
    if (!token) {
      Swal.fire('Token not found', 'Please login again.', 'error');
      return;
    ...
    const { value: editedComment } = await Swal.fire({
      ...
      inputValue: currentContent, // pre-fill with current comment content
    ...
    if (editedComment) {
      try {
        ...
        const response = await axiosInstance.put(`https://extra-brooke-yeremiadio-46b2183e.koyeb.app/api/comments/${documentId}`, {
          data: {
  ...
  const handleDeleteComment = async (documentId: string) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        const response = await axiosInstance.delete(`https://extra-brooke-yeremiadio-46b2183e.koyeb.app/api/comments/${documentId}`);
    ...
    <div className="container mx-auto p-6">
      {currentArticles.length > 0 ? (
        currentArticles.map((article) => (
          <div key={article.id} className="max-w-4xl mx-auto bg-white rounded-lg shadow-lg overflow-hidden mb-8">
            <img
            ...
            <div className="p-6">
              ...
              <div>
                <h3 className="text-xl font-medium text-gray-700 mb-3">Comments</h3>
                {article.comments.length > 0 ? (
                  article.comments.map((comment) => (
                    <div key={comment.id} className="flex items-start mb-4">
                      ...
                      <button
                        onClick={() => handleEditComment(comment.documentId, comment.content)}
